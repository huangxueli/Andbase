...
public class AbListViewHeader extends LinearLayout {
	...
	/** The tips of state nomal. */
	private String tipOfStateNomal = null;
	
	/** The tips of state Ready. */
	private String tipOfStateReady = null;
	...
	public void setState(int state) {
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(tipOfStateNomal!=null){
					tipsTextview.setText(tipOfStateNomal);
				}else{
			...
			case STATE_READY:
				if (mState != STATE_READY) {
					...
					if(tipOfStateReady!=null){
						tipsTextview.setText(tipOfStateReady);
					}else{
	...
	/**
	 * 设置状态STATE_NORMAL和STATE_Ready时的HeaderView的tip提示信息
	 * @param tipTexts
	 * @return
	 */
	public boolean setTipTextOfState(String[] texts){
		if(texts!=null && texts.length == 2){
			tipOfStateNomal = texts[0];
			tipOfStateReady = texts[1];
			return true;
		}else{
			return false;
		}
